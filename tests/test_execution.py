...
class Job_force_retry(Job):
    #This class imitates a job that fails once then passes on retry
    @property
    def status(self):
    ...
    def status(self, value):
        if (self.retries != self.num_retries) and (value == Status.RUNNING):
...
class TestRunState(TestCase):
    ...
    def _verify_serialized_state(self, _):
        ...
        for fs_jobstate, ex_jobstate in zip([value for key,value in fs_state.jobs.items()],
